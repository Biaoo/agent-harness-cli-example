...
def run_codex_checklist_judge(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="research-checklist-codex-") as temp_dir:
...
For each item, preserve the item_id and Criterion.
For each satisfied item, add concise Evidence.
For each unsatisfied item, add concise Evidence, Reason, and Suggestion.
